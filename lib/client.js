 ...
 *
 * Settings section "随身笔记": a prompt library with good use & management UX.
 *
 ...
 *
 * Data lives in the user data file (~/.dsh/dsh-notepad/prompts.md)
 * via host routes:
 *   GET /api/dsh-notepad/prompts → { ok, prompts, md }
 *   PUT /api/dsh-notepad/prompts  body { prompts } → { ok }
...
`
    ...
    // Legacy cache keys (pre-rename); read as a fallback so old browser
    // caches survive the rename and are re-persisted under the new key.
    const LEGACY_STORE_KEY = 'dsh.prompt-assistant.v1'
    const LEGACY_CATS_KEY = 'dsh.prompt-assistant.categories.v1'
    const API = '/api/dsh-notepad/prompts'
    ...
    function loadExtraCats() {
      try {
        const raw = localStorage.getItem(CATS_KEY) ?? localStorage.getItem(LEGACY_CATS_KEY)
        const arr = raw === null ? [] : JSON.parse(raw)
    ...
    const BADGETEXT = {
      ...
      file: { dot: 'var(--dsw-alias-state-success-primary,#16a34a)', text: '已保存', title: '数据已同步到 ~/.dsh/dsh-notepad/prompts.md' },
      ...
      return createElement(
        ...
        createElement(
          ...
          createElement('button', {
            ...
            title: '一键还原为插件内置的默认笔记。恢复前会先把当前数据备份为 prompts.backup-*.md。',
            style: {
        ...
        createElement(
          ...
          createElement('span', { className: 'dsh-pa-note', style: { marginLeft: 'auto' } },
            '数据存于用户目录 ~/.dsh/dsh-notepad/prompts.md'),
        ),
