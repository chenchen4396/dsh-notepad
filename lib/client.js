/**
 * dsh-notepad — browser half (runs inside the dsh web GUI).
 *
 * Settings section "随身笔记": a note library with good use & management UX.
 *
 * Use: search (`/` focuses, Esc clears), click content to copy (toast
 * feedback), copy button, expand/collapse long notes, Ctrl+Enter to save.
 * Manage: 「＋ 新建笔记」opens a modal dialog (Esc / backdrop / 取消 to
 * close), edit-in-card, two-step delete, reorder within a category (↑/↓,
 * persists to notes.json), rename categories in place, filter chips.
 * The header shows a compact storage-status dot (green = saved to file,
 * yellow = local cache) with the full path in its tooltip.
 *
 * Data lives in the user data file (~/.dsh/dsh-notepad/notes.json) via
 * host routes:
 *   GET  /api/dsh-notepad/notes → { ok, notes, json }
 *   PUT  /api/dsh-notepad/notes  body { notes } → { ok }
 *   POST /api/dsh-notepad/import   body { text, filename } → { ok, format, notes }
 *   POST /api/dsh-notepad/reset   → { ok, notes }
 * Import/export: 导出 downloads the library as a JSON file; 导入 passes a
 * foreign file (someone else's JSON export / plain note) to the host
 * parser, previews the result and merges or replaces.
...
`
    ...
    const STORE_KEY = 'dsh.notepad.v1'
    const API = '/api/dsh-notepad/notes'
    const RESET_URL = '/api/dsh-notepad/reset'
    const IMPORT_URL = '/api/dsh-notepad/import'

    let notes = loadLocalCache()
    ...
    function loadLocalCache() {
      try {
        ...
        console.error('[dsh-notepad] local cache load failed', error)
        return []
      }
    }
    ...
    function persistLocalCache() {
      try {
        ...
        console.error('[dsh-notepad] local cache save failed', error)
      }
    }
    ...
    /** Boot-time load: the file is the source of truth; the local cache is
     * only an offline fallback when the file cannot be reached. */
    async function initStore() {
      try {
        const fileList = await fetchFromFile()
        setStore(fileList, 'file')
      } catch (error) {
        console.error('[dsh-notepad] file load failed, falling back to local cache', error)
    ...
    function mutate(nextList) {
      ...
      return saveToFile(nextList).catch(error =>
        console.error('[dsh-notepad] file save failed', error))
    }

    function upsertNote(note) {
      const exists = notes.some(p => p.id === note.id)
      return mutate(exists ? notes.map(p => (p.id === note.id ? note : p)) : [...notes, note])
    }

    function removeNote(id) {
      return mutate(notes.filter(p => p.id !== id))
    }

    /** Move one note one step within its category (persisted order). */
    function moveNote(id, dir) {
      const i = notes.findIndex(p => p.id === id)
      const j = i + dir
      if (i < 0 || j < 0 || j >= notes.length) return null
      if (notes[j].category.trim() !== notes[i].category.trim()) return null
      const list = [...notes]
      ;[list[i], list[j]] = [list[j], list[i]]
      return mutate(list)
    }

    /** Rename a category across every note that uses it (and any empty
    ...
    function renameCategory(oldName, newName) {
      ...
      return mutate(notes.map(p =>
        p.category.trim() === from ? { ...p, category: to } : p))
    }

    // ── empty categories (persist in localStorage; notes.json stores only
    //    notes, so an empty category has no file entry) ──────────────────────

    const CATS_KEY = 'dsh.notepad.categories.v1'
    ...
    function computeChips() {
      const real = categoriesOf(notes)
      const chips = real.map(c => ({
        name: c,
        count: notes.filter(p => p.category.trim() === c).length,
      }))
    ...
    function loadExtraCats() {
      try {
        ...
        console.error('[dsh-notepad] categories load failed', error)
        return []
      }
    }
    ...
    function persistExtraCats() {
      try {
        ...
        console.error('[dsh-notepad] categories save failed', error)
      }
    }
    ...
    function addCategory(name) {
      const n = name.trim()
      if (n === '') return false
      const known = [...extraCats, ...categoriesOf(notes)]
    ...
    /**
     * Delete a category entirely: notes under it are removed too (real
     * categories cannot exist without notes in the md format). Empty
     * categories just lose their entry. Returns the persistence promise.
     */
    function deleteCategory(name) {
      const hasPrompts = notes.some(p => p.category.trim() === name)
      ...
      return mutate(notes.filter(p => p.category.trim() !== name))
    }
    ...
    const BADGETEXT = {
      ...
      file: { dot: 'var(--dsw-alias-state-success-primary,#16a34a)', text: '已保存', title: '数据已同步到 ~/.dsh/dsh-notepad/notes.json' },
      local: { dot: 'var(--dsw-alias-state-warn-primary,#d97706)', text: '本地缓存', title: '文件不可用，数据暂存在浏览器 localStorage' },
    }

    /** One note card: badge + title + actions + copyable content. */
    function NoteCard({ note, index, count, onCommit }) {
      ...
      const longEnough = chomp(note.content).length > 60 && note.content.split('\n').length > 3
      ...
      const copy = () => {
        copyText(note.content)
          .then(() => {
            ...
            showToast('已复制「' + (note.title === '（无标题）' ? note.category : note.title) + '」')
          })
          .catch(error => console.error('[dsh-notepad] copy failed', error))
      }
      ...
      const deleteClick = () => {
        ...
        onCommit(removeNote(note.id), () => showToast('已删除'))
      }
      ...
      return createElement(
        ...
        createElement(
          ...
          createElement('button', {
            className: 'dsh-pa-btn ghost',
            disabled: index === 0,
            onClick: () => onCommit(moveNote(note.id, -1)),
            title: '上移（保存到文件）',
    ...
    /** Open the "new note" dialog (optionally with a preset category). */
    function openNoteModal(categories, initialCategory) {
      openModal(close =>
        createElement(AddNoteModal, { categories, initialCategory: initialCategory ?? '', onClose: close }))
    }
    ...
    function AddNoteModal({ categories, initialCategory, onClose }) {
      ...
      return createElement(
        ...
        createElement(
          ...
          createElement(
            ...
            createElement('h3', { style: { margin: 0, fontSize: '15px', fontWeight: 650 } }, '新建笔记'),
            createElement('button', {
              className: 'dsh-pa-btn ghost',
              onClick: onClose,
              title: '关闭（Esc）',
    ...
    /** Modal dialog: confirm restoring the built-in default notes. */
    ...
    /** Open the import preview dialog for a parsed foreign note file. */
    function openImportModal(format, imported, filename) {
      openModal(close =>
        createElement(ImportModal, { format, imported, filename, onClose: close }))
    }

    /** Modal dialog: preview a foreign note file, pick merge or replace. */
    function ImportModal({ format, imported, filename, onClose }) {
      ...
      const fmtLabel = {
        json: 'JSON 笔记库（dsh-notepad 导出格式）',
        text: '单个笔记文件（文件名作为标题）',
      }[format] || String(format)

      const keyOf = p => `${p.category.trim() || '未分类'}\u0000${p.title}\u0000${p.content}`
      ...
      const confirm = () => {
        ...
        let next
        if (mode === 'merge') {
          const seen = new Set(notes.map(keyOf))
          for (const p of imported) {
            const key = keyOf(p)
            if (seen.has(key)) skipped += 1
            else { seen.add(key); added.push(p) }
          }
          if (added.length === 0) {
            showToast('没有新内容可导入（' + skipped + ' 条均与现有笔记重复）')
            onClose()
            return
          }
          next = [...notes, ...added]
        ...
        setBusy(true)
        const promise = mutate(next)
        const finish = () => {
          setBusy(false)
          onClose()
          if (mode === 'merge') {
            showToast(`已合并导入 ${added.length} 条` + (skipped > 0 ? `（跳过 ${skipped} 条重复）` : ''))
          } else {
            showToast(`已覆盖导入 ${imported.length} 条`)
          }
        }
        if (promise === null) finish()
        else promise.then(finish).catch(() => { setBusy(false); showToast('保存失败，请重试') })
      }
      ...
      const modeBtn = (value, label) =>
        createElement('button', {
          ...
          title: value === 'merge' ? '追加到现有笔记，重复内容自动跳过' : '清空现有笔记后只保留导入的内容',
          style: {
            ...btnBase, fontSize: '12px', padding: '6px 14px', borderRadius: '8px',
            background: mode === value ? T.btnFill : 'var(--dsw-alias-bg-layer-1,#ffffff)',
            color: mode === value ? '#fff' : T.muted,
            border: `1px solid ${mode === value ? T.btnFill : T.border}`,
            cursor: 'pointer',
          },
        }, label)

      const preview = imported.slice(0, 40)
      ...
      return createElement(
        ...
        createElement(
          'div',
          {
            className: 'dsh-pa-card',
            style: {
              width: 'min(500px, 100%)', background: T.card2,
              border: `1px solid ${T.border}`, borderRadius: '12px',
              boxShadow: 'var(--dsw-shadow-lv3,0 16px 48px rgba(15,23,42,.28))',
              padding: '20px', color: T.text,
            },
          ...
          createElement(
            ...
            createElement('h3', { style: { margin: 0, fontSize: '15px', fontWeight: 650 } }, '导入笔记'),
            createElement('button', {
              className: 'dsh-pa-btn ghost',
              onClick: onClose,
              title: '关闭（Esc）',
          ...
          createElement('p', { style: { margin: '0 0 10px', fontSize: '12.5px', lineHeight: 1.6, color: T.muted, wordBreak: 'break-all' } },
            '文件：', createElement('strong', { style: { color: T.text, fontWeight: 600 } }, filename), ' · ', fmtLabel),
          createElement('p', { style: { margin: '0 0 12px', fontSize: '13px' } },
            '识别到 ', createElement('strong', { style: { color: T.brand } }, imported.length), ' 条笔记。'),
          preview.length > 0 && createElement(
            'div',
            {
              style: {
                border: `1px solid ${T.borderSoft}`, borderRadius: '8px',
                maxHeight: '150px', overflowY: 'auto', padding: '6px 10px', marginBottom: '14px',
                background: 'var(--dsw-alias-bg-layer-1,#ffffff)', fontSize: '12px',
              },
            },
            ...preview.map(p => createElement(
              'div',
              { key: `${p.id}-${p.title}`, style: { display: 'flex', gap: '8px', padding: '3px 0' } },
              createElement('span', { style: { color: T.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' } }, p.title),
              createElement('span', { style: { flex: 'none', color: T.faint, fontSize: '11px' } }, p.category),
            )),
            imported.length > preview.length && createElement('p', { className: 'dsh-pa-note', style: { margin: '4px 0 0', textAlign: 'center' } },
              `… 还有 ${imported.length - preview.length} 条未显示`),
          ),
          createElement(
            'div',
            { style: { display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '16px' } },
            createElement('span', { style: { fontSize: '12.5px', color: T.muted } }, '导入方式：'),
            modeBtn('merge', '合并（推荐）'),
            modeBtn('replace', '覆盖全部'),
          ),
          createElement(
            'div',
            { style: { display: 'flex', gap: '8px', justifyContent: 'flex-end' } },
            createElement('span', { className: 'dsh-pa-note', style: { marginRight: 'auto' } }, '覆盖前请先导出备份 · Esc 关闭'),
            createElement('button', {
              className: 'dsh-pa-btn ghost',
              onClick: onClose,
              style: { ...btnBase, background: 'var(--dsw-alias-bg-layer-2,#f1f5f9)', color: T.text, padding: '7px 16px' },
            ...
            createElement('button', {
              ...
              onClick: confirm,
              disabled: busy,
              style: { ...btnBase, background: T.btnFill, color: '#fff', padding: '7px 20px', opacity: busy ? 0.7 : 1 },
            }, busy ? '导入中…' : '确认导入'),
          ),
        ),
      )
    ...
    function CategoryRow({ name, count, armed, onDelete, onRename }) {
      ...
      return createElement(
        ...
        }, armed ? `确认删除${count > 0 ? `（含 ${count} 条笔记）` : ''}？` : '删除'),
      )
    }
    ...
    function CategoryManageModal({ onClose }) {
      ...
      const del = (c, count) => {
        ...
        if (promise !== null) {
          promise.then(() => showToast(`已删除分类「${c}」及 ${count} 条笔记`)).catch(() => {})
        } else {
          showToast(`空分类「${c}」已删除`)
        }
      }
      ...
      return createElement(
        ...
        createElement(
          ...
          createElement('p', { className: 'dsh-pa-note', style: { margin: '14px 0 0' } },
            '删除非空分类会连同其中的笔记一并删除 · 点击「N 条 · ✎」可重命名'),
        ),
      )
    }
    /**
     ...
     * note library right under the input box, search, and put a note
     * into the composer draft with one click. The settings-page manager
     * stays the management surface.
     */
    function NotesQuickAccess({ inputActions, useInput }) {
      ...
      // Append the note to the composer draft, keeping what's already there.
      const useNote = p => {
      ...
      return createElement(
        ...
        createElement('button', {
          className: 'dsh-pa-btn ghost',
          onClick: () => setOpen(v => !v),
          title: '笔记库：搜索、按分类筛选并放入输入框（Esc 关闭）',
          'aria-label': '打开笔记库',
          style: {
        ...
        open && createElement(
          ...
          createElement(
            ...
            createElement('input', {
              ...
              placeholder: `🔍 搜索笔记…（共 ${list.length} 条）`,
          ...
          filtered.length === 0
            ? createElement(
                ...
                list.length === 0 ? '还没有笔记，去 设置 → 随身笔记 新建一条' : '没有匹配的笔记',
              )
            : createElement(
                'div',
    ...
    function NotesSection() {
      ...
      const doReset = () => {
        fetch(RESET_URL, { method: 'POST' })
          ...
          .then(data => {
            if (!data.ok) throw new Error(data.error || 'bad response')
            setStore(data.notes, 'file')
            setSavedAt(Date.now())
            showToast('已恢复默认内容（原数据已备份）')
          })
          .catch(error => {
            console.error('[dsh-notepad] reset failed', error)
      ...
      // ── export: download the whole library as a dsh-notepad JSON file ──
      const exportJson = () => {
        try {
          const blob = new Blob(
            [JSON.stringify({ version: 1, notes: list, exportedAt: new Date().toISOString() }, null, 2)],
            { type: 'application/json' })
          const url = URL.createObjectURL(blob)
          const a = document.createElement('a')
          a.href = url
          a.download = `dsh-notepad-${new Date().toISOString().slice(0, 10)}.json`
          document.body.appendChild(a)
          a.click()
          a.remove()
          setTimeout(() => URL.revokeObjectURL(url), 1000)
          showToast(`已导出 ${list.length} 条笔记（JSON）`)
        } catch (error) {
          console.error('[dsh-notepad] export failed', error)
          showToast('导出失败')
        }
      }

      // ── import: pick a foreign note file, host parses it, pick merge/replace ──
        ...
        reader.onload = () => {
          ...
          fetch(IMPORT_URL, {
            ...
            .then(data => {
              ...
              if (data.format === 'empty' || count === 0) {
                showToast('文件中没有可导入的笔记')
                return
              }
              if (data.format === 'unknown') {
                showToast('无法识别该文件（支持 dsh-notepad JSON 导出 / 任意文本或 Markdown 笔记文件）')
                return
              }
              openImportModal(data.format, data.notes, file.name)
            })
            .catch(err => {
              console.error('[dsh-notepad] import parse failed', err)
              showToast('导入失败：文件读取或解析出错')
            })
        }
      ...
      return createElement(
        ...
        createElement(
          ...
          createElement('h2', { style: { margin: 0, fontSize: '18px', fontWeight: 650 } }, '随身笔记'),
          createElement(
            'span',
            {
              title: badge.title,
        ...
        createElement(
          ...
          createElement('span', { style: { color: T.faint, fontSize: '11px' } },
            saving ? '⏳ 正在保存到 notes.json…' : savedAt !== null ? `已保存 ${timeLabel(savedAt)}` : ''),
        ),
        ...
        createElement(
          ...
          createElement('button', {
            className: 'dsh-pa-btn ghost',
            onClick: pickFile,
            title: '导入别人的笔记：支持 dsh-notepad JSON 导出 / 任意文本或 Markdown 笔记文件',
            style: { ...btnBase, background: 'transparent', color: T.muted },
          }, '📥 导入'),
          createElement('button', {
            className: 'dsh-pa-btn ghost',
            onClick: exportJson,
            title: '把全部笔记导出为 JSON 文件（可分享或导入到其他设备）',
            style: { ...btnBase, background: 'transparent', color: T.muted },
          }, '📤 导出'),
          createElement('button', {
            className: 'dsh-pa-btn danger',
            onClick: openResetConfirm,
            title: '一键还原为插件内置的默认笔记。恢复前会先把当前数据备份为 notes.json.backup-*。',
            style: {
        ...
        // hidden file picker for 导入 (triggered by the toolbar button)
        createElement('input', {
          ref: fileRef,
          type: 'file',
        ...
        createElement(
          ...
          ...chips.map(({ name: c, count }) => {
            ...
            return createElement(Chip, {
              ...
              onClick: () => {
                if (empty) {
                  // Clicking an empty category starts a note for it.
                  openNoteModal(unionCats, c)
                } else {
                  setFilter(c)
                }
              },
          ...
          createElement('span', { className: 'dsh-pa-note', style: { marginLeft: 'auto' } },
            '数据存于用户目录 ~/.dsh/dsh-notepad/notes.json'),
        ),
        ...
        list.length === 0
          ? createElement(
              ...
              createElement('p', { style: { margin: '0 0 12px' } }, '还没有笔记，先新建一条吧 ✍️'),
              createElement('button', {
                className: 'dsh-pa-btn primary',
                onClick: () => openNoteModal(unionCats),
    ...
    function Chip({ active, label, count, empty, onClick }) {
      return createElement('button', {
        className: 'dsh-pa-chip',
        onClick,
        title: empty ? '空分类：点击直接为该分类新建笔记' : undefined,
        style: {
    ...
    function apply(ctx) {
      if (claimed) return
      claimed = true
      ctx.effect(() => { claimed = false }, 'notepad: apply claim')
      ...
      try {
        ...
        // as dsh-sticky-note) opening the note library under the input box;
        // one click appends a note to the draft.
        ctx.slots.inject('conversation.input.left', () =>
          ctx.slots.register({
            name: 'conversation.input.left',
            id: 'notepad',
            order: 30,
          }, (zoneProps) => createElement(NotesQuickAccess, {
        ...
        ctx.effect(() => window.removeEventListener('storage', onStorage), 'notepad: storage sync')
      } catch (error) {
        console.error('[dsh-notepad] apply failed', error)
      }
    }
