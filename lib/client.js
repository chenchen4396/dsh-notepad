...
`
    ...
    function PromptQuickAccess({ inputActions, useInput }) {
      ...
      const filtered = q === '' ? list : list.filter(p =>
        chomp(p.title).toLowerCase().includes(q) || chomp(p.content).toLowerCase().includes(q))
      ...
      return createElement(
        ...
        createElement('button', {
          ...
          createElement('span', { style: { fontSize: '13px', lineHeight: 1 } }, '📋'),
          '提示词',
        ),
        open && createElement(
          'div',
