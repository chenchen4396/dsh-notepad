// dsh-prompt-assistant — host half (runs in the dsh host process).
//
// Owns the single source of truth for prompt data: a Markdown file named
// prompts.md in the USER data directory:
//
//   $DSH_HOME/dsh-prompt-assistant/prompts.md   (default: ~/.dsh/…)
//
...
const PROMPTS_FILE = join(DSH_HOME, 'dsh-prompt-assistant', 'prompts.md')
...
async function ensureDataFile() {
  ...
  try {
    ...
    console.log('[dsh-prompt-assistant] seeded prompts.md from package', SEED_FILE)
  } catch {
...
/** The two routes this plugin contributes to the web server. */
function makeRoutes() {
  return [
    {
      kind: 'exact',
      path: '/api/dsh-prompt-assistant/prompts',
      handler: (req, res) => {
...
export const name = 'prompt-assistant'
export const inject = ['webServer']

export function apply(ctx) {
  console.log('[dsh-prompt-assistant] host half loaded, prompts.md at', PROMPTS_FILE)
